...
relevance에 3배 가중치를 줘서 해결했다. 다만 이 값은 하나의 실패 사례를 기준으로 정한 운영값이었고, 전체 랭킹 품질에서 최적이라는 근거는 없었다.

이후 하루 50건짜리 풀 6일치(300건)에 사람이 직접 0/1/2 라벨을 매겨 nDCG@10으로 재보고 **5배로 바꿨다**(2026-09-22). ×3에서 0.593, ×5에서 0.663, ×8에서 0.655였고, LLM 점수만 쓰면 0.499로 무작위(0.522)보다도 낮았다. 계산 근거는 `backend/app/pipeline/rank.py`의 주석에 남겨뒀다.

이 실패 사례를 추적한 전체 과정은 블로그 글 [RRF로 추천 랭킹을 "더 공정하게" 바꿨더니 오히려 나빠졌다](https://velog.io/@daewook10_04/Tech-Radarv1)에 정리했다. LLM 채점 자체가 본문 유무에 끌려가던 문제와 그 수정은 [LLM 채점은 '모르겠다'를 '낮다'로 적는다](https://velog.io/@daewook10_04/Tech-Radarv2)에 있다.
