...
# 오히려 예전보다 관심사 매칭이 흐려지는 걸 실측으로 확인함(2026-08-26).
#
# 2026-09-22: 하루 50건짜리 풀 6일치(300건)에 사람이 직접 0/1/2 라벨을 매겨 재보니
# relevance 3배로도 모자랐다. nDCG@10이 ×3에서 0.593, ×5에서 0.663, ×8에서 0.655,
# LLM 점수만 쓰면 0.499로 무작위(0.522)보다도 낮았다. 라벨과의 상관도 relevance만
# 6일 내내 양수였고(중앙값 +0.16) 나머지 셋은 0 근처에서 부호가 오갔다.
# ×8이나 relevance 단독으로 더 밀지 않은 이유는, 관심사 밖으로 관심이 퍼진 날
# (2026-09-22가 그랬다) 그쪽이 더 크게 무너지기 때문. ×5가 평소에 가장 좋으면서
# 그런 날에도 덜 무너지는 지점이었다. 평가셋과 계산은 docs/eval/ (로컬 전용).
_SIGNAL_WEIGHTS = {"relevance": 5, "importance": 1, "novelty": 1, "credibility": 1}
