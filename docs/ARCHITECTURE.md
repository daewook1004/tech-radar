...
### 아직 미결정/미구현
- **랭킹 가중치와 채점 프롬프트 재검토** — 사람이 매긴 라벨로 재보니 관련도만 사람 판단과 꾸준히 같은 방향이고(일별 +0.17~+0.42), LLM 세 점수는 0 근처에서 부호가 오간다. `relevance`를 3배가 아니라 5배로 두는 쪽이 3일 연속 더 나았지만 차이가 작아 확정 전. 채점 프롬프트에 관심사를 넣어 "이 사람이 읽을 가치"를 묻는 안도 후보. 라벨 250건이 모이면 결정 (`docs/eval/`)
